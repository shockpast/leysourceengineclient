...
bool leychan::NET_BufferToBufferDecompress(char* dest, unsigned int& destLen, char* source, unsigned int sourceLen)
{
	...
	if (s.IsCompressed((unsigned char*)source))
	{
		...
		{
			WarningLog("NET_BufferToBufferDecompress with improperly sized dest buffer (%u in, %u needed)\n", destLen, uDecompressedLen);
			return false;
		}
		else
	...
	if (_Log_PacketHeaders)
	{
		if (message.GetNumBitsLeft() > 0)
		{
			...
			if (flags & PACKET_FLAG_COMPRESSED)
				DebugLog("COMPRESSED _ ");

			if (flags & PACKET_FLAG_ENCRYPTED)
			{
				DebugLog("ENCRYPTED _ %i _ ", msgsize);
			}
...
#define IGNORE_CRC
	...
	if (_ShouldChecksumPackets)
	{
		...
		{
			DebugLog("corrupted packet %i at %i\n", sequence, m_nInSequenceNr);
			return -1;
		}
#endif
	}
	...
	if (flags & PACKET_FLAG_CHALLENGE)
	{
		...
		{
			ErrorLog("Bad challenge, discared: %i\n", nChallenge);
			return -1;
		}
	...
	if (sequence <= m_nInSequenceNr)
	{
		if (_ShowDrop)
			if (sequence == m_nInSequenceNr)
				DebugLog("duplicate packet %i at %i\n", sequence, m_nInSequenceNr);
			else
				DebugLog("out of order packet %i at %i\n", sequence, m_nInSequenceNr);
	}
	...
	{
		if (_ShowAllDrop)
			DebugLog("Dropped %i packets at %i\n", m_PacketDrop, sequence);

		if (_MaxPacketDrop > 0 && m_PacketDrop > _MaxPacketDrop)
		{
			if (_ShowAllDrop)
				DebugLog("Too many dropped packets (%i) at %i\n", m_PacketDrop, sequence);
...
bool leychan::ReadSubChannelData(bf_read& buf, int stream)
{
	...
	if (offset == 0) // first fragment, read header info
	{
		...
		if (bSingleBlock)
		{
			if (buf.ReadOneBit())
			{
				...
				DebugLog("DATA IS COMPRESSED, UNCOMPRESSED: %i\n", data->nUncompressedSize);
			}
			else
				data->isCompressed = false;
		...
		else
		{
			...
			if (buf.ReadOneBit())
			{
				data->isCompressed = true;
				data->nUncompressedSize = buf.ReadUBitLong(MAX_FILE_SIZE_BITS);
				DebugLog("DATA IS COMPRESSED, UNCOMPRESSED, !SINGLE: %i\n", data->nUncompressedSize);
			}
			else {
				data->isCompressed = false;
			}
		...
		if (data->buffer)
		{
			...
			delete[] data->buffer;
			WarningLog("Fragment transmission aborted at %i/%i.\n", data->ackedFragments, data->numFragments);
		}
	...
	else
	{
		if (data->buffer == NULL)
		{
			...
			//  for a retry
			WarningLog("Received fragment out of order: %i/%i\n", startFragment, numFragments);
			return false;
		}
	}
...
bool leychan::CheckReceivingList(int nList)
{
	...
	if (data->ackedFragments > data->numFragments)
	{
		ErrorLog("Receiving failed: too many fragments %i/%i\n", data->ackedFragments, data->numFragments);
		return false;
	}
	...
	if (_ShowFragments)
		DebugLog("Receiving complete: %i fragments, %i bytes\n", data->numFragments, data->bytes);
	...
	else
	{
		...
		if (!fileexists(data->filename))
		{
			...
			if (lastslash)
			{
				...
				if (err)
					ErrorLog("Could not create dir: %d\n", err);
			}

			// open new file for write binary
			data->file = fopen(data->filename, "wb");

			DebugLog("Received buffer data: %s\n", data->buffer);
			...
			if (0 != data->file)
			{
				...
				if (_ShowFragments)
				{
					Log("FileReceived: %s, %i bytes (ID %i)\n", data->filename, data->bytes, data->transferID);
				}
			}
			else
			{
				ErrorLog("Failed to write received file '%s'!\n", data->filename);
			}
		}
		else
		{
			// don't overwrite existing files
			WarningLog("Download file '%s' already exists!\n", data->filename);
		}
	}
...
void leychan::CheckWaitingList(int nList)
{
	...
	if (data->ackedFragments == data->numFragments)
	{
		...
		if (_ShowFragments)
			DebugLog("Sending complete: %i fragments, %i bytes.\n", data->numFragments, data->bytes);
	...
	{
		DebugLog("CheckWaitingList: invalid acknowledge fragments %i/%i.\n", data->ackedFragments, data->numFragments);
	}
}
...
{
	DebugLog("leychan::SetSignonState Should do SetSignonState  %i, %i\n", state, servercount);

	if (state == 2 && servercount == -1)
	{
		WarningLog("Server requested us to reconnect, trying to reconnect... (net_SignOnState)\n");
	...
	{
		DebugLog("leychan::SetSignonState Ignored signonstate %d we are on %d\n", state, this->m_iSignOnState);
		return;
	}
	...
	if (state == 3)
	{
		DebugLog("leychan::SetSignonState Received SignonState 3!\n");
...
void leychan::ProcessServerInfo(unsigned short protocolversion, int count)
{
	...
	DebugLog("ProcessServerInfo\n");
}

void leychan::Reconnect()
{
	// TODO: actually try to reconnect
	// WarningLog("Server requested us to reconnect.\n");
}